...
int contar_coincidencias_matriz(int **, int, int, int);
int suma_vector(int[], int);
int simetria_matriz(int**, int, int);
...
int contar_coincidencias_matriz(int **M, int m, int n, int num)
{
   int i, j;
   ...
   if(M[m-1][n-1]==num)
      return 1+contar_coincidencias_matriz(M, m-1, n, num)+
             contar_coincidencias_matriz(M, m, n-1, num)-
             contar_coincidencias_matriz(M, m-1, n-1, num);
   else
      return contar_coincidencias_matriz(M, m-1, n, num)+
             contar_coincidencias_matriz(M, m, n-1, num)-
             contar_coincidencias_matriz(M, m-1, n-1, num);
}
...
int suma_vector(int v[], int n)
{
  ...
  return suma_vector(v, n - 1) + v[n - 1];
}
// [OPCIONAL] simetria_matriz
// Completar por el alumno
// ...

int simetria_matriz(int **M, int m, int n)
{
  if (m == 0 && n == 0)
  {
    return 1;
  }
  if (M[m - 1][n - 1] == M[n - 1][m - 1])
  {
    return simetria_matriz(M, m - 1, n) && simetria_matriz(M, m, n - 1);
  }
  else
    return 0;
}
